...
async function createSeedArticle({ title, description, content, author_id, editor_id }) {
    try {
        const { rows: [article] } = await client.query(`
        INSERT INTO articles(title, description, content, author_id, editor_id)
        VALUES($1, $2, $3, $4, $5)
        RETURNING *;
        `, [title, description, content, author_id, editor_id]);
        return article;
    } catch (error) {
        throw error;
    }
...
async function createArticle(body, author_id) {
    try {
        const { rows: [article] } = await client.query(`
...
async function updateArticleById(article_id, fields = {}, editor_id) {
    ...
    try {
        const values = [...Object.values(fields), editor_id, article_id];
        const { rows: [article] } = await client.query(`
        UPDATE articles
        SET ${setString}, "editor_id"=$${Object.keys(fields).length + 1}
